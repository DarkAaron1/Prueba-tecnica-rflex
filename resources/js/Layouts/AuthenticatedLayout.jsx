...
export default function AuthenticatedLayout({ header, children }) {
    ...
    // Definimos una constante para limpiar el JSX
            ...
            <nav className="border-b border-gray-100 bg-white">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="flex h-16 justify-between">
                            ...
                            <div className="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                ...
                                {isAdmin && (
                                    <>
                                        ...
                                        <NavLink href={route('holdings.index')} active={route().current('holdings.*')}>
                                            Holdings
                                        </NavLink>
                                ...
                                {/* --- VISTA DE EMPLEADO / GENERAL --- */}
                                {!isAdmin && (
                                    <NavLink href={route('my.schedule')} active={route().current('my.schedule')}>
                                        Mi Horario
                                    </NavLink>
                                )}
                            </div>
                        ...
                        {/* Dropdown de Usuario (Perfil y Logout) */}
                                    ...
                                    <Dropdown.Trigger>
                                        <span className="inline-flex rounded-md">
                                            <button type="button" className="inline-flex items-center rounded-md border border-transparent bg-white px-3 py-2 text-sm font-medium leading-4 text-gray-500 transition duration-150 ease-in-out hover:text-gray-700 focus:outline-none">
                                                {user.name} 
                                                <span className="ml-1 text-[10px] bg-gray-100 px-2 py-0.5 rounded uppercase font-bold text-gray-400">
                                                    {user.role}
                                                </span>
                        ...
                        <div className="-me-2 flex items-center sm:hidden">
                            <button onClick={() => setShowingNavigationDropdown((previousState) => !previousState)} className="inline-flex items-center justify-center rounded-md p-2 text-gray-400 transition duration-150 ease-in-out hover:bg-gray-100 hover:text-gray-500 focus:outline-none">
